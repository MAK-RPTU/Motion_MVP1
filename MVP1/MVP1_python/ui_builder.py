...
class UIBuilder:
            ...
            with ui.VStack(spacing=8, height=0):
                ...
                with ui.ScrollingFrame(height=220, style=get_style()):
                    with ui.VStack(spacing=4):
                        self._chat_display_container = ui.VStack(spacing=4)
    ...
    def _on_chat_submit(self):
        text = self._chat_input_field.model.get_value_as_string().strip()
        if not text:
            return

        # Append user message
        with self._chat_display_container:
            ui.Label(f"User: {text}")

        self._chat_input_field.model.set_value("")

        # Process prompt through controller
        response = self._chat_controller.handle_prompt(text)
